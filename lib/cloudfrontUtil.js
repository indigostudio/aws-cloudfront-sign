 ...
 *
 * @param {String} cfUrl resource URL
 * @param {Object} params parameters
 * @return {String} Signed CloudFront URL
 */
function getSignedUrl(cfUrl, params) {
  const signedPolicy = getSignedPolicy(cfUrl, params);
  return appendPolicy(cfUrl, signedPolicy);  
}

/**
 * Appends a policy as the query string of the provided URL.
 *
 * @param {String} cfUrl resource URL
 * @param {Object} signedPolicy a signed policy as returned by getSignedPolicy
 * @return {String} Signed CloudFront URL
 */
function appendPolicy(cfUrl, signedPolicy) {
  ...
  _.extend(parsedUrl.query, {
    'Expires': signedPolicy.expireTime,
    'Policy': signedPolicy.policyStr,
    'Signature': signedPolicy.signature,
    'Key-Pair-Id': signedPolicy.keypairId
  });
...
function getSignedCookies(cfUrl, params){
  ...
  var cookies = {};
  cookies['CloudFront-Policy'] = signedPolicy.policy;
  cookies['CloudFront-Signature'] = signedPolicy.signature;
  cookies['CloudFront-Key-Pair-Id'] = signedPolicy.keypairId;
  return cookies;
}

/**
 * Get a signed policy to access a resource for an URL
 *
 * @param {String} cfUrl resource URL
 * @param {Object} params parameters
 * @return {String} Signed policy
 */
function getSignedPolicy(cfUrl, params) {
...
exports.appendPolicy = appendPolicy;
